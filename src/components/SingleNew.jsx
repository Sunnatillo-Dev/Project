...
function SingleNew({
  ...
  }, [id, userId, isSaved, refresh]);
  let router = useRouter();
  let isInSavedPage = router.asPath == "/saved";
  let getNewsData = (id) => {
    router.push(`/new/${id}`);
  };
  return (
    ...
    >
      <Box>
        ...
        <Box
          ...
          lineHeight="24px"
        >
          {description.split("").slice(0, 104).join("")} ...
        </Box>
        <Box
          justifyContent={"space-between"}
          width={"100%"}
          ...
          <Box>
            ...
            {isInSavedPage && (
              <Button
                ...
                opacity={0.5}
                onClick={
                  user ? () => onDelete(id) : () => router.push("/sign-in")
                }
                _hover={{ opacity: 0.8 }}
              >
